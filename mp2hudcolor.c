...
static const unsigned TWGC_MAGIC_OFFSET = 0x25C3;
...
int main(int argc, char *argv[])
{
    ...
    // Read file to RAM
    size_t result = fread(buff, NTWK_FILE_SIZE, 1, src);
    fclose(src);
    if (result != 1)
    {
    ...
    for (unsigned i = 0; i < sizeof(TWGC_IDS) / 4; i++)
    {
        ...
        {
            // Check for ID
            uint8_t* data = buff + TWGC_MAGIC_OFFSET + j;
